...
void chunkWorkerEntryPoint(ChunkWorkerInitParams params) async {
  ...
  try {
    while (currentOffset <= params.endByte && retryAttempts < maxWorkerRetries) {
      dio.options.headers['Range'] = 'bytes=$currentOffset-${params.endByte}';
    ...
    // Ensure all sockets are properly closed when isolate work terminates
    isolateClient.close(force: true);
  }
}
